...
class CNNFeatureExtractor(FeatureExtractor):
    ...
    def __init__(self, input_shape_dims : tuple):
        self.flag_fit = False
        ...
        base_model  = keras.applications.vgg16.VGG16(include_top=False,input_shape=shape)
        for i in range(len(base_model.layers)):
            base_model.layers[i].trainable = False
        self.model.add(base_model)
        #self.model.add( keras.layers.GlobalAveragePooling2D() )
        #self.model.add( keras.layers.GlobalMaxPooling2D() )
        self.model_preprocess = keras.applications.vgg16.preprocess_input
        self.im_preproc_type = "rescale"
    ...
    def adjust_image(self, image : np.array, im_type : str = None) -> np.array:
        """
        ...
        elif (im_type == "rescale"):
            image_resized = cv2.resize(image, shape, None)
        elif (im_type == "letterbox"):
            nrow, ncol, nchan = image.shape
            max_dim = max(nrow, ncol)
            image_padded = np.zeros((max_dim, max_dim, nchan),dtype="uint8")
            begin_row = (max_dim - nrow) // 2
            begin_col = (max_dim - ncol) // 2
            end_row = (max_dim + nrow) // 2
            end_col = (max_dim + ncol) // 2
            image_padded[begin_row : end_row, begin_col: end_col, :] = image[:,:,:]
            image_resized = cv2.resize(image_padded, shape, None)
        else:
            image_resized = None
    ...
    def extract(self, image : np.array) -> np.array :
        ...
        """
        image_prep              = self.adjust_image(image, im_type=self.im_preproc_type)
    ...
    def extract_batch(self, images_list : list) -> np.array:
        ...
        for i in range(n_images):
            images_array[i,:,:,:] = self.adjust_image(images_list[i], im_type=self.im_preproc_type)
